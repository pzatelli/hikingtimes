...
#pendenza e pendenza_gradi
	for i in range (0,numero-1):
		...
		gscript.run_command('db.execute', sql="UPDATE " + str(mappa) +  "_2 SET pendenza=(-1)*" + str(math.atan2(temp,dist)) + " WHERE cat="+str(i+2))
...
#velocita
	...
	gscript.run_command('db.execute', sql="UPDATE " + str(mappa) +  "_2 SET velocita=" + str(d) + " WHERE pendenza_gradi>25 AND pendenza_gradi<=90")
	gscript.run_command('db.execute', sql="UPDATE " + str(mappa) +  "_2 SET velocita=" + str(e) + " WHERE pendenza_gradi>-80 AND pendenza_gradi<=-45")
...
#estrazione tempo di percorrenza
	...
	gscript.run_command('db.execute', sql="UPDATE sottorete_definitiva3 SET tempo_ritorno="+str(t)+" WHERE cat="+str(numero_arco))
